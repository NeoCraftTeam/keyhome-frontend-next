...
import { adReportsService, AdReportReason, AdReportScamReason, CreateAdReportPayload } from '@/services/ad-reports.service';
...
const reasonOptions: Option<AdReportReason>[] = [
  { value: 'inaccurate', label: 'Elle est inexacte ou incorrecte', icon: <ReportProblemOutlinedIcon fontSize="small" /> },
  { value: 'not_real_property', label: 'Ce n\'est pas un veritable logement', icon: <HomeWorkOutlinedIcon fontSize="small" /> },
  { value: 'scam', label: 'C\'est une arnaque', icon: <GppBadOutlinedIcon fontSize="small" /> },
  { value: 'shocking_content', label: 'Le contenu est choquant', icon: <WarningAmberOutlinedIcon fontSize="small" /> },
  { value: 'other', label: 'Il s\'agit d\'autre chose', icon: <ReportProblemOutlinedIcon fontSize="small" /> },
];

const scamOptions: Option<AdReportScamReason>[] = [
  { value: 'asked_off_platform_payment', label: 'L\'hote m\'a demande de payer en dehors de KeyHome', hint: 'Ex : especes, virement ou transfert bancaire', icon: <PaymentsOutlinedIcon fontSize="small" /> },
  { value: 'shared_contacts', label: 'L\'hote a partage ses coordonnees', hint: 'Ex : numero personnel, email personnel', icon: <ContactPhoneOutlinedIcon fontSize="small" /> },
  { value: 'promoting_external_services', label: 'L\'hote fait la promotion de services externes', hint: 'Ex : liens vers des sites externes', icon: <CampaignOutlinedIcon fontSize="small" /> },
  { value: 'duplicate_listing', label: 'Il s\'agit d\'une annonce en double', hint: 'Ex : copie d\'une autre annonce', icon: <ReportProblemOutlinedIcon fontSize="small" /> },
  { value: 'misleading_listing', label: 'Elle est trompeuse', hint: 'Ex : photos non conformes ou informations mensongeres', icon: <WarningAmberOutlinedIcon fontSize="small" /> },
];
...
export default function AdReportModal({
  ...
  const submit = async (payload: CreateAdReportPayload) => {
    ...
    try {
      ...
      if (status === 429) {
        onServerErrorChange('Trop de tentatives. Veuillez patienter un instant puis reessayer.');
        return;
      }
      ...
      const rawMessage =
        ...
        fallback;
      const safePatterns = ['SQLSTATE', 'An email must have a "To", "Cc", or "Bcc" header.'];
      const shouldMaskMessage = safePatterns.some((pattern) => rawMessage.includes(pattern));
  ...
  return (
    ...
    >
      <DialogTitle sx={{ pb: 0, pt: 1.5, px: { xs: 2, md: 3 }, minHeight: 48 }}>
        <IconButton
          aria-label="Fermer"
          onClick={onClose}
          disabled={submitting}
      ...
      <DialogContent sx={{ px: { xs: 2, md: 4 }, pb: 0, pt: { xs: 1.25, md: 2 }, display: 'flex', flexDirection: 'column' }}>
        <AnimatePresence mode="wait">
          {step === 'reason' && (
            <motion.div key="reason" {...panelMotion}>
              <Typography variant="h4" fontWeight={700} sx={{ fontSize: { xs: '1.7rem', md: '2.1rem' }, mb: 1 }}>
          ...
          {step === 'scam' && (
            <motion.div key="scam" {...panelMotion}>
              <Typography variant="h4" fontWeight={700} sx={{ fontSize: { xs: '1.8rem', md: '2.2rem' }, mb: 2 }}>
                ...
                {scamOptions.map((option) => (
                  ...
                  >
                    <Box sx={{ display: 'flex', gap: 1.25, minWidth: 0 }}>
                      ...
                      <Box>
                        <Typography variant="h6" sx={{ fontWeight: 500, fontSize: { xs: '1.02rem', md: '1.28rem' } }}>
                          {option.label}
                        </Typography>
                        {option.hint && (
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1.25 }}>
                {paymentMethods.map((method) => (
                  <Chip
                    ...
                    variant={selectedPaymentMethods.includes(method.value) ? 'filled' : 'outlined'}
                    sx={{
                      borderRadius: 999,
                      px: 0.5,
          ...
          {step === 'done' && (
            <motion.div key="done" {...panelMotion}>
              ...
              <Typography variant="body1" sx={{ lineHeight: 1.7, mb: 2 }}>
                Merci d&apos;avoir pris le temps de nous expliquer ce qui se passe. Nos equipes vont
                analyser cette annonce rapidement.
              </Typography>
        ...
        <Box sx={{ mt: 'auto', pt: 2.5 }}>
          ...
          <Divider />
          <Box sx={{ py: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 1.5 }}>
            <Button
              variant="text"
