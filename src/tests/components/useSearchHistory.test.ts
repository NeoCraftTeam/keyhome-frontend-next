...
describe('useSearchHistory', () => {
  ...
  it('adds a search term', () => {
    const { result } = renderHook(() => useSearchHistory());
    act(() => {
      result.current.addSearch('Yaoundé');
    });
    expect(
      result.current.history.some(
        (h: SearchHistoryItem) => h.query === 'Yaoundé'
      )
    ).toBe(true);
  });
  ...
  it('does not add duplicates', () => {
    const { result } = renderHook(() => useSearchHistory());
    act(() => {
      result.current.addSearch('Douala');
    });
    act(() => {
      result.current.addSearch('Douala');
    });
    expect(
      result.current.history.filter(
        (h: SearchHistoryItem) => h.query === 'Douala'
      ).length
    ).toBe(1);
  });
  ...
  it('removes a search term', () => {
    const { result } = renderHook(() => useSearchHistory());
    act(() => {
      result.current.addSearch('Bastos');
    });
    act(() => {
      result.current.removeSearch('Bastos');
    });
    expect(
      result.current.history.some(
        (h: SearchHistoryItem) => h.query === 'Bastos'
      )
    ).toBe(false);
  });
  ...
  it('limits history to max items (8 by default)', () => {
    ...
    for (let i = 0; i < 15; i++) {
      act(() => {
        result.current.addSearch(`City ${i} name`);
      });
    }
  ...
  it('filters suggestions by query', () => {
    ...
    act(() => {
      result.current.addSearch('Yaoundé Bastos');
    });
    const suggestions = result.current.getSuggestions('yaou');
    expect(suggestions.length).toBe(2);
    expect(
      suggestions.every((s: SearchHistoryItem) =>
        s.query.toLowerCase().includes('yaou')
      )
    ).toBe(true);
  });
  ...
  it('persists to localStorage', () => {
    const { result } = renderHook(() => useSearchHistory());
    act(() => {
      result.current.addSearch('Kribi');
    });
    const stored: SearchHistoryItem[] = JSON.parse(
      localStorage.getItem('keyhome_search_history') || '[]'
    );
