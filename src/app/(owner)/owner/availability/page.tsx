...
export default function OwnerAvailabilityPage() {
  ...
  const { data: adsData, isLoading: adsLoading } = useQuery({
    ...
    queryFn: () => ownerService.getMyAds({ per_page: 100, sort: 'created_at', order: 'desc' }),
  });

  const ads = ((adsData as any)?.data ?? []) as Ad[];
  ...
  const createMutation = useMutation({
    mutationFn: (payload: AvailabilityPayload) => ownerService.createAvailability(selectedAdId, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['owner-availability', selectedAdId] });
      setDialogOpen(false);
      resetForm();
      setSnackbar({ message: 'Planning créé avec succès', severity: 'success' });
    },
    onError: () => setSnackbar({ message: 'Erreur lors de la création', severity: 'error' }),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: Partial<AvailabilityPayload> }) =>
    ...
    onError: () => setSnackbar({ message: 'Erreur lors de la mise à jour', severity: 'error' }),
  });

  const deleteMutation = useMutation({
    mutationFn: (scheduleId: string) => ownerService.deleteAvailability(selectedAdId, scheduleId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['owner-availability', selectedAdId] });
      setDeleteConfirm(null);
      setSnackbar({ message: 'Planning supprimé', severity: 'success' });
    },
    onError: () => setSnackbar({ message: 'Erreur lors de la suppression', severity: 'error' }),
  });
  ...
  const openEdit = (s: AvailabilitySchedule) => {
    ...
    setForm({
      ...
      ends_on: s.ends_on,
      periods: s.periods.map((p) => ({ starts_at: p.starts_at, ends_at: p.ends_at })),
      recurrence: (s.frequency as AvailabilityPayload['recurrence']) ?? 'once',
      recurrence_days: (s.frequency_config as any)?.days ?? [],
  ...
  const addPeriod = () => {
    if (form.periods.length < 4) {
      setForm((f) => ({ ...f, periods: [...f.periods, { starts_at: '14:00', ends_at: '17:00' }] }));
    }
  };
  ...
  const updatePeriod = (idx: number, field: 'starts_at' | 'ends_at', value: string) => {
    setForm((f) => ({
      ...f,
      periods: f.periods.map((p, i) => (i === idx ? { ...p, [field]: value } : p)),
    }));
  };
  ...
  return (
    ...
    <Container maxWidth="md" sx={{ py: { xs: 2, md: 4 } }}>
      ...
      <Typography color="text.secondary" sx={{ mb: 3 }}>
        Définissez vos créneaux horaires pour les visites de vos biens. Les visiteurs pourront réserver un créneau disponible.
      </Typography>
      ...
      {!selectedAdId ? (
        <Card sx={{ borderRadius: 3, border: '1px dashed', borderColor: 'divider', p: 6, textAlign: 'center' }}>
          ...
          <Typography color="text.secondary">
            Choisissez une annonce ci-dessus pour gérer ses créneaux de visite.
          </Typography>
        </Card>
      ) : schedulesLoading ? (
        ...
        <>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            <Typography variant="subtitle1" fontWeight={700}>
              Plannings ({(schedules as AvailabilitySchedule[] | undefined)?.length ?? 0})
            </Typography>
            <Button
              variant="contained"
              size="small"
          ...
          {!(schedules as AvailabilitySchedule[] | undefined)?.length ? (
            <Card sx={{ borderRadius: 3, border: '1px dashed', borderColor: 'divider', p: 4, textAlign: 'center' }}>
              <EventIcon sx={{ fontSize: 40, color: 'text.disabled', mb: 1 }} />
              <Typography variant="body1" fontWeight={600} gutterBottom>
                Aucun planning
              </Typography>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                Créez un planning de disponibilité pour que les visiteurs puissent réserver un créneau.
              </Typography>
              <Button
                variant="outlined"
                startIcon={<AddIcon />}
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 1 }}>
                      <Box sx={{ flex: 1, minWidth: 0 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
                          ...
                          {s.is_active && (
                            <Chip label="Actif" size="small" color="success" sx={{ fontSize: '0.65rem', height: 20 }} />
                          )}
                        </Box>
                        <Typography variant="body2" color="text.secondary">
                          {s.starts_on} → {s.ends_on || 'Indéfini'}
                          {s.frequency && ` · ${RECURRENCE_LABELS[s.frequency] || s.frequency}`}
                        </Typography>
      ...
      <Dialog
        open={dialogOpen}
        onClose={() => { setDialogOpen(false); setEditingSchedule(null); }}
        maxWidth="sm"
        fullWidth
        PaperProps={{ sx: { borderRadius: 3 } }}
      >
        <DialogTitle fontWeight={700}>
          {editingSchedule ? 'Modifier le planning' : 'Nouveau planning de disponibilité'}
        </DialogTitle>
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: '8px !important' }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            <DatePicker
              ...
              onChange={(d) =>
                setForm((f) => ({
                  ...f,
                  starts_on: d && isValid(d) ? format(d, 'yyyy-MM-dd') : f.starts_on,
                }))
              }
              slotProps={{
          ...
          <FormControl size="small">
            ...
            <Select
              ...
              label="Récurrence"
              onChange={(e) => setForm((f) => ({ ...f, recurrence: e.target.value as AvailabilityPayload['recurrence'] }))}
            >
              ...
              <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap' }}>
                {Object.entries(DAYS_LABELS).map(([key, label]) => (
                  <Chip
                    ...
                    label={label}
                    size="small"
                    variant={form.recurrence_days?.includes(key) ? 'filled' : 'outlined'}
                    color={form.recurrence_days?.includes(key) ? 'primary' : 'default'}
          ...
          <Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
              ...
              {form.periods.length < 4 && (
                <Button size="small" startIcon={<AddIcon />} onClick={addPeriod} sx={{ textTransform: 'none', fontSize: '0.75rem' }}>
                  Ajouter
                </Button>
              )}
            </Box>
              ...
              {form.periods.map((p, idx) => (
                <Box key={idx} sx={{ display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  {form.periods.length > 1 && (
                    <IconButton size="small" onClick={() => removePeriod(idx)} color="error">
        ...
        <DialogActions sx={{ p: 2 }}>
          <Button
            onClick={() => { setDialogOpen(false); setEditingSchedule(null); }}
            variant="outlined"
            disabled={isPending}
            sx={{ borderRadius: 2, textTransform: 'none' }}
          >
          ...
          <Button
            ...
            variant="contained"
            disabled={isPending || !form.name || !form.starts_on || form.periods.length === 0}
            sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 700 }}
          >
            {isPending ? 'Enregistrement…' : editingSchedule ? 'Mettre à jour' : 'Créer le planning'}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary">
            Les réservations en attente associées à ce planning seront automatiquement annulées.
          </Typography>
        </DialogContent>
        <DialogActions sx={{ p: 2 }}>
          <Button onClick={() => setDeleteConfirm(null)} variant="outlined" sx={{ borderRadius: 2, textTransform: 'none' }}>
            Annuler
          </Button>
          <Button
      ...
      >
        {snackbar ? (
          <Alert onClose={() => setSnackbar(null)} severity={snackbar.severity} sx={{ borderRadius: 2 }}>
            {snackbar.message}
          </Alert>
        ) : undefined}
      </Snackbar>
