  ...
  const updateMutation = useMutation({
    mutationFn: async ({
      ...
      if (imagesToDelete?.length) {
        imagesToDelete.forEach((mid) => formData.append('images_to_delete[]', String(mid)));
      }
      ...
      if (tourScenes && tourScenes.length > 0) {
        ...
        const existingScenes = tourScenes.filter((s) => s.id && !s.id.startsWith('new-') && !s.file);
    ...
    onSuccess: () => {
      ...
      setSnackbar({ message: 'Annonce mise à jour avec succès', severity: 'success' });
    },
    onError: () => {
  ...
  const contractMutation = useMutation({
    mutationFn: () =>
      ownerService.generateLeaseContract(id, {
        ...
        monthly_rent: contractForm.monthly_rent ? Number(contractForm.monthly_rent) : undefined,
        deposit_amount: contractForm.deposit_amount ? Number(contractForm.deposit_amount) : undefined,
    ...
    onSuccess: () => {
      ...
      setSnackbar({ message: 'Contrat de bail généré avec succès', severity: 'success' });
    },
    onError: () => {
      setSnackbar({ message: 'Erreur lors de la génération du contrat', severity: 'error' });
    },
  });
    ...
    return (
      <Container maxWidth="md" sx={{ py: 4 }}>
        <Alert
          ...
          sx={{ borderRadius: 2 }}
          action={
            <Button color="inherit" size="small" onClick={() => router.push('/owner/ads')}>
              Retour
            </Button>
          }
        >
  ...
  return (
    <Container maxWidth="md" sx={{ py: { xs: 2, md: 4 } }}>
        ...
        >
          <Box>
            ...
            <Box sx={{ display: 'flex', gap: 0.75, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              />
              {ad.is_visible === false && (
                <Chip label="Masqué" size="small" color="secondary" variant="outlined" />
              )}
              {ad.has_3d_tour && (
                <Chip
          ...
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
            <ButtonGroup variant="outlined" size="small" sx={{ '& .MuiButton-root': { borderRadius: 2, textTransform: 'none', fontWeight: 600 } }}>
              <Button
                startIcon={ad.is_visible !== false ? <HiddenIcon /> : <VisibleIcon />}
                onClick={() => toggleMutation.mutate()}
                disabled={toggleMutation.isPending}
              >
                {ad.is_visible !== false ? 'Masquer' : 'Afficher'}
              </Button>
              {ad.status !== AdStatus.PENDING && ad.status !== AdStatus.DECLINED && (
                <>
                  {ad.status !== AdStatus.RESERVED && (
                    <Button
                      onClick={() => setStatusMutation.mutate(AdStatus.RESERVED)}
                  ...
                  {ad.status !== AdStatus.AVAILABLE && (
                    <Button
                      onClick={() => setStatusMutation.mutate(AdStatus.AVAILABLE)}
                      disabled={setStatusMutation.isPending}
                    >
                      Disponible
                    </Button>
            ...
            {(ad.status === AdStatus.AVAILABLE || ad.status === AdStatus.RESERVED) && (
              <Button
                variant="outlined"
                color="primary"
                size="small"
                ...
                onClick={() => {
                  setContractForm((prev) => ({
                    ...
                    deposit_amount: ad.deposit_amount ? String(ad.deposit_amount) : ad.price != null ? String(ad.price) : '',
                  }));
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            Cette action est irréversible. L&apos;annonce &quot;{ad.title}&quot; sera définitivement
            supprimée, ainsi que toutes les photos, le tour 3D et les données associées.
          </DialogContentText>
        </DialogContent>
      ...
      >
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: '8px !important' }}>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
            Les informations du bien &quot;{ad.title}&quot; seront automatiquement pré-remplies dans le contrat.
          </Typography>
          <TextField
          ...
          <TextField
            ...
            onChange={(e) => setContractForm((p) => ({ ...p, tenant_email: e.target.value }))}
          />
          <TextField
            label="N° CNI / Passeport"
            size="small"
            value={contractForm.tenant_id_number}
            onChange={(e) => setContractForm((p) => ({ ...p, tenant_id_number: e.target.value }))}
          />
          <TextField
            label="Référence du logement"
            size="small"
            value={contractForm.unit_reference}
            onChange={(e) => setContractForm((p) => ({ ...p, unit_reference: e.target.value }))}
          />
          <TextField
            ...
            onChange={(e) => setContractForm((p) => ({ ...p, lease_start: e.target.value }))}
          ...
          <FormControl size="small" required>
            ...
            <Select
              ...
              label="Durée du bail"
              onChange={(e) => setContractForm((p) => ({ ...p, lease_duration_months: Number(e.target.value) }))}
            >
          ...
          <TextField
            ...
            onChange={(e) => setContractForm((p) => ({ ...p, deposit_amount: e.target.value }))}
          />
          <TextField
            ...
            onChange={(e) => setContractForm((p) => ({ ...p, special_conditions: e.target.value }))}
          />
          {contractForm.special_conditions.trim() && (
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  const enhanced = await ownerService.enhanceLeaseConditions(contractForm.special_conditions);
                  setContractForm((p) => ({ ...p, special_conditions: enhanced }));
                } finally {
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Vérifiez les informations avant de générer le contrat PDF. Vous pourrez modifier si nécessaire.
          </Typography>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 1, borderBottom: '1px solid', borderColor: 'divider' }}>
              <Typography variant="body2" color="text.secondary">Bien</Typography>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 1, borderBottom: '1px solid', borderColor: 'divider' }}>
              ...
              <Typography variant="body2" fontWeight={600}>{contractForm.tenant_name}</Typography>
            </Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 1, borderBottom: '1px solid', borderColor: 'divider' }}>
              <Typography variant="body2" color="text.secondary">Téléphone</Typography>
              <Typography variant="body2" fontWeight={600}>{contractForm.tenant_phone}</Typography>
            </Box>
            {contractForm.tenant_email && (
              <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 1, borderBottom: '1px solid', borderColor: 'divider' }}>
                <Typography variant="body2" color="text.secondary">Email</Typography>
                <Typography variant="body2" fontWeight={600}>{contractForm.tenant_email}</Typography>
              </Box>
            )}
            {contractForm.tenant_id_number && (
              <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 1, borderBottom: '1px solid', borderColor: 'divider' }}>
                <Typography variant="body2" color="text.secondary">N° CNI / Passeport</Typography>
                <Typography variant="body2" fontWeight={600}>{contractForm.tenant_id_number}</Typography>
              </Box>
            )}
            {contractForm.unit_reference && (
              <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 1, borderBottom: '1px solid', borderColor: 'divider' }}>
                <Typography variant="body2" color="text.secondary">Référence logement</Typography>
                <Typography variant="body2" fontWeight={600}>{contractForm.unit_reference}</Typography>
              </Box>
            )}
            <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 1, borderBottom: '1px solid', borderColor: 'divider' }}>
              <Typography variant="body2" color="text.secondary">Début du bail</Typography>
              <Typography variant="body2" fontWeight={600}>
                {new Date(contractForm.lease_start).toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' })}
              </Typography>
            </Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 1, borderBottom: '1px solid', borderColor: 'divider' }}>
              <Typography variant="body2" color="text.secondary">Durée</Typography>
              <Typography variant="body2" fontWeight={600}>{contractForm.lease_duration_months} mois</Typography>
            </Box>
            {contractForm.monthly_rent && (
              <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 1, borderBottom: '1px solid', borderColor: 'divider' }}>
                <Typography variant="body2" color="text.secondary">Loyer mensuel</Typography>
                <Typography variant="body2" fontWeight={600}>{Number(contractForm.monthly_rent).toLocaleString('fr-FR')} XAF</Typography>
              </Box>
            )}
            {contractForm.deposit_amount && (
              <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 1, borderBottom: '1px solid', borderColor: 'divider' }}>
                <Typography variant="body2" color="text.secondary">Caution</Typography>
                <Typography variant="body2" fontWeight={600}>{Number(contractForm.deposit_amount).toLocaleString('fr-FR')} XAF</Typography>
              </Box>
            )}
        ...
        <DialogActions sx={{ p: 2, gap: 1 }}>
          ...
          <Button
            ...
            sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 700 }}
          >
            {contractMutation.isPending ? 'Génération…' : 'Confirmer et générer le PDF'}
          </Button>
        </DialogActions>
      </Dialog>
